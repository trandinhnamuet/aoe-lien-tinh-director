 ...
 */
export type GameDurations = { p1: (number | null)[]; p2: (number | null)[] } | null;

export async function saveMatchResult(
  matchId: string, s1: number, s2: number, status: MatchStatus,
  m1: number | null = null, m2: number | null = null, gameDurations: GameDurations = null,
): Promise<MatchStatus> {
  ...
  await sql.begin(async (tx) => {
    ...
    await tx`update aoe.matches set player1_score=${s1}, player2_score=${s2}, player1_machine=${m1}, player2_machine=${m2},
      game_durations=${gameDurations ? sql.json(gameDurations) : null}, status=${finalStatus}, winner_id=${winner} where id=${matchId}`;
