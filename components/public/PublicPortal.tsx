...
import type {
  ClusterSnapshot, RoundSnapshot, GroupVM, SwissLegVM, BracketColumnVM, BracketNodeVM,
} from "@/lib/types";
    ...
    <div style={{ position: "relative", minHeight: "100vh", background: "#080b1e", fontFamily: "'IBM Plex Sans',sans-serif", color: "#e8eeff", overflowX: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 10 }}>
          ...
          <div style={{ maxWidth: 1280, margin: "0 auto", padding: "14px 24px", display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: "#9aaad8", letterSpacing: 2, marginTop: 2 }}>CSDN STUDIO · DIRECTOR</div>
              </div>
            </div>
            ...
            <div style={{ position: "relative" }}>
              <button onClick={() => setSelOpen((v) => !v)} style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(93,182,255,.1)", border: "1px solid rgba(93,182,255,.4)", color: "#e8eeff", padding: "8px 14px", borderRadius: 8, cursor: "pointer", fontFamily: "'IBM Plex Sans',sans-serif", fontSize: 13 }}>
                <span style={{ color: "#9aaad8" }}>{snap.tournament.name} ·</span>
              ...
              {selOpen && (
                <div style={{ position: "absolute", top: 46, left: 0, minWidth: 280, background: "#0e1336", border: "1px solid #2c3470", borderRadius: 10, boxShadow: "0 18px 50px rgba(0,0,0,.5)", padding: 8, zIndex: 50 }}>
                  <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: "#9aaad8", letterSpacing: 1, padding: "6px 10px" }}>CHỌN CỤM · {snap.tournament.name}</div>
                    ...
                    return (
                      <button key={c.id} onClick={() => loadCluster(c.id)} style={{ width: "100%", textAlign: "left", cursor: "pointer", padding: 10, borderRadius: 8, border: "1px solid transparent", color: "#e8eeff", background: active ? "rgba(93,182,255,.12)" : "transparent" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                          ...
                          <span style={{ fontFamily: FONT_MONO, fontSize: 9, letterSpacing: 1, padding: "2px 7px", borderRadius: 4, ...(live ? { color: "#a9d6ff", background: "rgba(93,182,255,.15)", border: "1px solid rgba(93,182,255,.4)" } : { color: "#9aaad8", background: "rgba(255,255,255,.05)" }) }}>{live ? "ĐANG THI ĐẤU" : "ĐÃ XONG"}</span>
                        </div>
                        <div style={{ fontSize: 11, color: "#9aaad8", marginTop: 3 }}>{c.date} · {c.location}</div>
                      </button>
                    );
                  })}
                </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 7, background: "rgba(93,182,255,.12)", border: "1px solid rgba(93,182,255,.5)", padding: "7px 13px", borderRadius: 30 }}>
              <span style={{ width: 7, height: 7, borderRadius: "50%", background: isLive ? "#5db6ff" : "#8890b8", boxShadow: isLive ? "0 0 8px #5db6ff" : "none", animation: isLive ? "liveDot 1.4s infinite" : "none" }} />
        ...
        <footer style={{ borderTop: "1px solid rgba(93,182,255,.12)", padding: "22px 24px", textAlign: "center" }}>
          <span style={{ fontFamily: FONT_MONO, fontSize: 11, color: "#7884a8", letterSpacing: 1 }}>{snap.tournament.name} {snap.tournament.year} · CSDN Studio</span>
        </footer>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap", marginBottom: 22 }}>
        <div>
          <div style={{ fontFamily: FONT_MONO, fontSize: 11, color: "#9aaad8", letterSpacing: 2, marginBottom: 8 }}>{snap.cluster.date} · {snap.cluster.location}</div>
        ...
        <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(93,182,255,.16)", borderRadius: 14, padding: "20px 22px", marginBottom: 26 }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 16, flexWrap: "wrap", marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
              ...
              <span style={{ fontFamily: FONT_MONO, fontSize: 12, color: "#9aaad8" }}>{snap.currentRound.configText}</span>
            </div>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 30 }}>
        {snap.rounds.map((r) => (
          <button key={r.id} onClick={() => onOpenRound(r.id)} style={{ flex: 1, minWidth: 220, textAlign: "left", background: "rgba(255,255,255,.03)", border: "1px solid rgba(93,182,255,.16)", borderRadius: 12, padding: "16px 18px", cursor: "pointer", color: "#e8eeff" }}>
            ...
            <div style={{ fontSize: 12, color: "#9aaad8", marginTop: 4 }}>{r.configText}</div>
          </button>
        ))}
      </div>
    </div>
...
function GroupCard({ g }: { g: GroupVM }) {
  ...
  return (
    <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(93,182,255,.16)", borderRadius: 14, padding: 18, animation: "cardIn .5s both" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: cols, gap: 8, padding: "0 10px 6px", fontFamily: FONT_MONO, fontSize: 10, color: "#9aaad8", letterSpacing: 1 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
        {g.rows.map((r) => (
          <div key={r.playerId} style={{ display: "grid", gridTemplateColumns: cols, gap: 8, alignItems: "center", padding: "7px 10px", borderRadius: 8, background: r.advance ? "rgba(93,182,255,.1)" : "transparent", border: `1px solid ${r.advance ? "rgba(93,182,255,.28)" : "transparent"}` }}>
            <span style={{ fontFamily: FONT_SAIRA, fontWeight: 800, fontStyle: "italic", fontSize: 17, color: r.advance ? "#5db6ff" : "#9aaad8" }}>{r.rank}</span>
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ fontFamily: FONT_MONO, fontSize: 9, color: "#9aaad8" }}>{mach(r.machine)}</div>
            </div>
            ...
            {g.showDraw && <span style={{ ...numCell, color: "#9aaad8" }}>{r.draw}</span>}
            <span style={{ ...numCell, color: "#9aaad8" }}>{r.diff > 0 ? "+" : ""}{r.diff}</span>
          </div>
        ))}
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 5, marginTop: 10 }}>
          {g.matches.map((m, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: 6, alignItems: "center", fontSize: 13 }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontFamily: FONT_SAIRA, fontWeight: m.aWin ? 700 : 500, fontSize: 14, color: m.bWin ? "#8890b8" : "#e8eeff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.aName}</div>
                {m.aName !== m.aNick && <div style={{ fontFamily: FONT_SAIRA, fontWeight: 400, fontSize: 11, color: m.bWin ? "#6a7090" : "#9bd8ff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.aNick}</div>}
              </div>
              <span style={{ fontFamily: FONT_MONO, fontWeight: 600, fontSize: 13, padding: "0 10px", color: m.live ? "#9bd8ff" : m.done ? "#5db6ff" : "#9aaad8" }}>{m.score}</span>
              <div style={{ minWidth: 0, textAlign: "right" }}>
                <div style={{ fontFamily: FONT_SAIRA, fontWeight: m.bWin ? 700 : 500, fontSize: 14, color: m.aWin ? "#8890b8" : "#e8eeff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.bName}</div>
                {m.bName !== m.bNick && <div style={{ fontFamily: FONT_SAIRA, fontWeight: 400, fontSize: 11, color: m.aWin ? "#6a7090" : "#9bd8ff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.bNick}</div>}
              </div>
            </div>
          ))}
        </div>
    ...
    <div>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
        ...
        <div style={{ flex: 1, minWidth: 240, background: "rgba(255,255,255,.03)", border: "1px solid #2c3470", borderRadius: 12, padding: "16px 18px" }}>
          <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: "#8890b8", letterSpacing: 1, marginBottom: 8 }}>✕ ĐÃ BỊ LOẠI</div>
          <div style={{ fontFamily: FONT_SAIRA, fontWeight: 700, fontSize: 16, lineHeight: 1.5, color: "#8890b8" }}>{round.eliminated.length ? round.eliminated.join(" · ") : "Chưa có"}</div>
        </div>
      </div>
            ...
            {leg.matches.map((m) => {
              ...
              return (
                <div key={m.id} style={rowStyle}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: done && m.winner === "a" ? 700 : 500, color: done && m.winner === "b" ? "#8890b8" : "#fff" }}>
                      <div style={{ fontFamily: FONT_SAIRA, fontSize: 16, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.aName}</div>
                      {m.aName !== m.aNick && <div style={{ fontFamily: FONT_SAIRA, fontWeight: 400, fontSize: 12, color: done && m.winner === "b" ? "#6a7090" : "#9bd8ff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.aNick}</div>}
                    </div>
                    <div style={{ fontFamily: FONT_MONO, fontSize: 9, color: "#9aaad8", marginTop: 2 }}>{mach(m.aMachine)} · {m.aRec}</div>
                  </div>
                  <span style={{ fontFamily: FONT_SAIRA, fontStyle: "italic", fontWeight: 800, fontSize: 22, color: live ? "#9bd8ff" : done ? "#5db6ff" : "#9aaad8" }}>{m.score}</span>
                  <div style={{ minWidth: 0, textAlign: "right" }}>
                    <div style={{ fontWeight: done && m.winner === "b" ? 700 : 500, color: done && m.winner === "a" ? "#8890b8" : "#fff" }}>
                      <div style={{ fontFamily: FONT_SAIRA, fontSize: 16, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.bName}</div>
                      {m.bName !== m.bNick && <div style={{ fontFamily: FONT_SAIRA, fontWeight: 400, fontSize: 12, color: done && m.winner === "a" ? "#6a7090" : "#9bd8ff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.bNick}</div>}
                    </div>
                    <div style={{ fontFamily: FONT_MONO, fontSize: 9, color: "#9aaad8", marginTop: 2 }}>{mach(m.bMachine)} · {m.bRec}</div>
                  </div>
                </div>
              );
            })}
...
function BracketView({ round }: { round: Extract<RoundSnapshot, { type: "knockout_single" | "knockout_multi" }> }) {
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 14, alignItems: "stretch", overflowX: "auto", paddingBottom: 12 }}>
        {round.columns.map((col: BracketColumnVM, ci) => {
          const isFinal = /chung kết/i.test(col.name);
          return (
            <div key={col.name} style={{ flex: "none", width: isFinal ? 230 : 210, display: "flex", flexDirection: "column", justifyContent: "space-around", gap: 14 }}>
              <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: isFinal ? "#9bd8ff" : "#9aaad8", letterSpacing: 2, textAlign: "center" }}>{isFinal ? "★ " : ""}{col.name.toUpperCase()}</div>
              {col.nodes.map((n, ni) => <BracketNode key={n.id ?? `${ci}-${ni}`} n={n} emphasize={isFinal} delay={0.2 + ci * 0.15 + ni * 0.08} />)}
            </div>
          );
        })}
      </div>
...
function BracketNode({ n, emphasize, delay }: { n: BracketNodeVM; emphasize?: boolean; delay: number }) {
  const done = n.status === "done", live = n.status === "live";
  const base: React.CSSProperties = { position: "relative", borderRadius: 10, padding: "11px 13px", minWidth: 184, transition: "box-shadow .4s, border-color .4s", animation: `cardIn .5s ${delay}s both` };
  const box: React.CSSProperties = live ? { ...base, background: "rgba(93,182,255,.06)", animation: `${base.animation}, pulseIce 2s 1s infinite` }
    : done ? { ...base, background: "rgba(255,255,255,.05)", border: "1px solid #2c3470" }
    : { ...base, background: "rgba(255,255,255,.02)", border: "1px dashed #2c3470" };
  if (emphasize) box.boxShadow = "inset 0 0 0 1.5px rgba(93,182,255,.5)";

  const renderRow = (name: string | null, nick: string | null, machine: number | null, score: number | null, win: boolean, dim: boolean, top: boolean) => (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: top ? 6 : 0, opacity: dim ? 0.5 : 1 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 7, minWidth: 0, flex: 1 }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontFamily: FONT_SAIRA, fontWeight: 700, fontSize: 16, color: win ? "#5db6ff" : name ? "#fff" : "#7884a8", textShadow: win ? "0 0 12px rgba(93,182,255,.6)" : undefined, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{name ?? "Chờ KQ"}</div>
          {name && nick && name !== nick && <div style={{ fontFamily: FONT_SAIRA, fontWeight: 400, fontSize: 11, color: win ? "#9bd8ff" : "#9aaad8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{nick}</div>}
        </div>
        {name && machine !== null && <span style={{ fontFamily: FONT_MONO, fontSize: 9, color: "#9aaad8", flexShrink: 0 }}>M{String(machine).padStart(2, "0")}</span>}
      </div>
      <span style={{ fontFamily: FONT_SAIRA, fontStyle: "italic", fontWeight: 800, fontSize: 19, color: win ? "#5db6ff" : "#e8eeff" }}>{name && (done || live) && score !== null ? score : ""}</span>
    </div>
  );

  return (
    <div style={box}>
      {renderRow(n.aName, n.aNick, n.aMachine, n.aScore, done && n.winner === "a", done && n.winner === "b", true)}
      {renderRow(n.bName, n.bNick, n.bMachine, n.bScore, done && n.winner === "b", done && n.winner === "a", false)}
    </div>
  );
}

function Empty({ text }: { text: string }) {
  return <div style={{ gridColumn: "1/-1", padding: "40px 20px", textAlign: "center", color: "#9aaad8", fontFamily: FONT_MONO, fontSize: 13, border: "1px dashed #2c3470", borderRadius: 12 }}>{text}</div>;
}
