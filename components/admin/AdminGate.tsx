...
export default function AdminGate({ children }: { children: React.ReactNode }) {
  ...
  if (authed === null) {
    return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", color: "#aab6e0", fontFamily: FONT_MONO }}>Đang tải…</div>;
  }
        ...
        <form onSubmit={submit} style={{ width: "100%", maxWidth: 380, background: "rgba(10,14,40,.7)", border: "1px solid rgba(93,182,255,.2)", borderRadius: 16, padding: 28, boxShadow: "0 18px 50px rgba(0,0,0,.5)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: "#aab6e0", letterSpacing: 2 }}>TRANG QUẢN TRỊ</div>
            </div>
          </div>
          <div style={{ fontFamily: FONT_MONO, fontSize: 10, letterSpacing: 1, color: "#aab6e0", marginBottom: 6, textTransform: "uppercase" }}>Mật khẩu admin</div>
          ...
          <div style={{ fontSize: 11, color: "#8a92ba", marginTop: 14, textAlign: "center" }}>Chỉ hỏi 1 lần trên thiết bị này.</div>
        </form>
      </div>
    );
  }
