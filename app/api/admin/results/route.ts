...
export async function POST(req: NextRequest) {
  try {
    ...
    const { matchId, s1, s2, status, m1, m2, gameDurations } = body as {
      matchId: string; s1: number; s2: number; status: MatchStatus; m1: number | null; m2: number | null;
      gameDurations: { p1: (number | null)[]; p2: (number | null)[] } | null;
    };
    if (!matchId) return Response.json({ ok: false, error: "Thiếu matchId" }, { status: 400 });
    const finalStatus = await saveMatchResult(matchId, s1, s2, status, m1 ?? null, m2 ?? null, gameDurations ?? null);
